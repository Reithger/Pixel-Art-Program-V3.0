...
import visual.settings.page.DrawingPage;
import visual.settings.page.FilePage;
...
//---  Constructors   -------------------------------------------------------------------------
	...
	public SettingsBar(int x, int y, int wid, int hei, WindowFrame ref) {
		...
		ref.addPanelToWindow(getMenuBarWindowName(), "menu_bar", menu);
...
//---  Operations   ---------------------------------------------------------------------------
	...
	private void formatPages(int x, int y, int wid, int hei) {
		for(Page p : PAGES) {
			...
			reference.addPanelToWindow(p.getName(), p.getName(), p);
			p.drawPage();
		}
	}
	...
	private void drawMenuBar() {
		...
		for(int i = 0; i < PAGES.length; i++) {
			menu.handleText("text_title_" + i, false, posX, menu.getHeight() / 2, distX, menu.getHeight(), MENU_FONT, PAGES[i].getName());
